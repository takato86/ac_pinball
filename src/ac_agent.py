...
class ActorCrticiAgent(object):
    def __init__(self, action_space, observation_space, basis_order=3, epsilon=0.01, gamma=0.99, lr_critic=0.01, lr_qlearn=0.01):
        ...
        # parameters
        self.w_q = np.zeros((action_space.n, self.n_features))
        # Hyper parameters
        self.epsilon = epsilon
        self.gamma = gamma
        self.lr_critic = lr_critic
        self.lr_qlearn = lr_qlearn
        self.actor = Actor(action_space.n, self.n_features)
    ...
    def act(self, observation):
        ...
        action_dist = actor.get_action_dist(feature)
        self.vis_action_dist = action_dist
        try:
            action = np.random.choice(list(range(self.action_space.n)), 1, p=action_dist)
        except ValueError:
